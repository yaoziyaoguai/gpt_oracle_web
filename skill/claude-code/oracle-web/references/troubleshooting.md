...
- `ChatGPT session not detected` or `Login button detected on page`: the configured Profile copy did not produce an authenticated ChatGPT page. Run `oracle-web --doctor` and compare its wrapper, user-data root, and Profile with the user's expected browser identity. If they match, report the authentication failure; do not probe another Profile, pass `--copy-profile`, or substitute Oracle's private `~/.oracle/browser-profile` directory. The failure message ends with a `(probe: ...)` summary; use its `sessionStatus`, `sessionResolved`, `backendStatus`, and `cfBlocked` fields to distinguish a missing session from a blocked probe before reporting.
...
6. Run `oracle-web --doctor` when wrapper or Chrome Profile drift is suspected. The output checks paths and required files only; `loginState=not-tested` is not authentication evidence.
7. Run `scripts/verify.sh` from the repository when installation drift is suspected.
