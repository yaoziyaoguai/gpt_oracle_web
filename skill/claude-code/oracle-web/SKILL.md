...
- Treat the current slider's ARIA position as the source of truth. Menu text may list Pro 5/5 while the active control remains at Instant 1/5; that menu text alone is not selection evidence. The supported five-position UI allows only position 4 or position 5.
...
- Recheck the active effort after attachments finish and before sending. A composer remount may reset it; if position 4/5 cannot be confirmed again, stop without submitting.
